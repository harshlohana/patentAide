...
function LoginForm() {
  ...
  let LoginUser = (event) => {
    ...
    if (userLogin.username === "" || userLogin.userpass === "") {
      ...
      const user = {
        CustomerLoginID: userLogin.username,
        CustomerPassword: userLogin.userpass,
      };

      try {
        axios
          .post(`${serverString}Customer/login`, user)
          .then((res) => {
            if (res.data !== "Invalid Credentials") {
              ...
              localStorage.setItem(
                UserNameKey,
                res.data.customer.CustomerFullName
              );
  ...
  return (
    <>
      <HomeNavBar goToHomePage={goToHomePage} current={"loginPage"} />
      <div className="login-page">
        <div className="form">
          <form className="login-form">
            ...
            <p className="message">
              <span onClick={showForgotPassswordPage}>Forgot Password?</span>
            </p>
            <p className="message">
              Not registered?{" "}
              <span onClick={showRegistrationPage}>Create an account</span>
            </p>
          </form>
        </div>
      </div>
