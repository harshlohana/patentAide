...
//------------------------------------------------------------------------
    ...
    function fixForFooterNoContent() {
        if (($('.footer-content').html().replace(/\s/g, '') == '') || ($('.footer-content').html().replace(/\s/g, '') == '<divclassName="footer-logo-divider"></div><divclassName="footer-social-divider"></div>'))
        {
    ...
    function loadPortfolioItemContent(portfolioItemID) {
        $.ajax({
            ...
            success: function (html) {
                ...
                $('.portfolio-load-content-holder').append('<div id="pcw-' + portfolioItemID + '" className="portfolio-content-wrapper">' + getPortfolioItemHtml + '</div>');
                if (!$("#pcw-" + portfolioItemID + " .close-icon").length) {
                    $("#pcw-" + portfolioItemID).prepend('<div className="close-icon"></div>');
                }
                $('html, body').animate({scrollTop: $('#portfolio-wrapper').offset().top}, 400);
                setTimeout(function () {
